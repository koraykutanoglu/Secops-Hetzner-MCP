...
function getToken() {
    ...
    if (!token) {
        throw new Error('HCLOUD_TOKEN environment variable is not set. ' +
...
async function hcloudRequest(method, path, body) {
    ...
    if (!res.ok) {
        ...
        throw new Error(`Hetzner API error ${res.status}: ${err.code ?? res.statusText} — ${err.message ?? JSON.stringify(data)}`);
    }
    return data;
}
function sshExecute(host, command, options) {
...
const tools = [
    ...
    {
        name: 'create_server',
        description: 'Create (provision) a new Hetzner Cloud server. ' +
    ...
    {
        name: 'delete_server',
        description: 'Permanently destroy a Hetzner Cloud server by name or numeric ID. ' +
    ...
    {
        name: 'ssh_execute',
        description: 'Execute a shell command on a Hetzner Cloud server over SSH. ' +
...
async function handleListServers() {
    const data = await hcloudRequest('GET', '/servers');
...
async function handleCreateServer(args) {
    const name = args.name;
    const serverType = args.server_type ?? 'cx22';
    const image = args.image ?? 'ubuntu-24.04';
    const location = args.location ?? 'nbg1';
    const sshKeyNames = args.ssh_key_names;
    const userData = args.user_data;
    ...
    const data = await hcloudRequest('POST', '/servers', body);
    const s = data.server;
    return {
        id: s.id,
        name: s.name,
...
async function handleDeleteServer(args) {
    const nameOrId = args.name_or_id;
    ...
    if (/^\d+$/.test(nameOrId)) {
        ...
        const data = await hcloudRequest('GET', `/servers?name=${encodeURIComponent(nameOrId)}`);
        if (!data.servers.length) {
            throw new Error(`Server '${nameOrId}' not found. Use list_servers to see available servers.`);
        }
...
async function handleListServerTypes() {
    const data = await hcloudRequest('GET', '/server_types');
...
async function handleListSshKeys() {
    const data = await hcloudRequest('GET', '/ssh_keys');
...
async function handleSshExecute(args, onLine) {
    const nameOrId = args.name_or_id;
    ...
    const usePty = args.use_pty ?? false;
    ...
    let host;
    if (/^\d+$/.test(nameOrId)) {
        const data = await hcloudRequest('GET', `/servers/${nameOrId}`);
...
// MCP Server
// ---------------------------------------------------------------------------
const server = new Server({ name: 'SecopsHetznerMCP', version: '0.1.0' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    async function sendProgress(line) {
